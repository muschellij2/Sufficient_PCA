...
```{r prcomp, cache=TRUE, dependson="data_prcomp"}
system.time({
    pc_comp = prcomp(
      x = Z, 
      center = FALSE,
...
```{r svd_z, cache=TRUE, dependson="data_prcomp"}
system.time({
    svd_z = svd(Z, nu = 0)
    pc_svd_z = Z %*% svd_z$v
})
...
```{r equiv, cache=TRUE, dependson="data_prcomp"}
...
```{r pc_cov_z, cache=TRUE, dependson="data_prcomp"}
system.time({
    cov = t(Z) %*% Z
...
```{r left_is_right, cache=TRUE, dependson="data_prcomp"}
svd_cov_all = svd(cov)
...
```{r split, cache = TRUE, dependson="data_prcomp"}
n_ids = 5
...
```{r combine, cache=TRUE, dependson = "split"}
res = make_cov(stats)
split_cov = res$C
...
```{r pc_cov_split, cache=TRUE, dependson="data_prcomp"}
system.time({
